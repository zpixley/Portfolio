...
def breadth_first_tree_search(problem):
    ...
    while frontier:
        if max_nodes < len(frontier):
            max_nodes = len(frontier)
...
def id_astar_search(problem, h=None):
    h = memoize(h or problem.h, 'h')
    ...
    def id_dfs(node, cost_so_far, explored, problem, limit):
        if cost_so_far + h(node) > limit:
            return False, cost_so_far + h(node)
        if problem.goal_test(node.state):
            return True, node
        min = infinity
        for child in node.expand(problem):
            if child not in explored:
                explored.add(child)
                found, n = id_dfs(child, problem.path_cost(cost_so_far, node.state, None, child.state), explored, problem, limit)
                if found:
                    return True, n
                if n < min:
                    min = n
    ...
    n = 0
    while n != infinity:
        found, n = id_dfs(root, 0, explored, problem, bound)
        if found: return n
        bound = n
