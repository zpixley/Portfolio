...
def degree(assignment, csp):
    """Degree heuristic based on number of neighbors"""
    choice = argmin_random_tie(
        [v for v in csp.variables if v not in assignment],
        key=lambda var: num_legal_values(csp, var, assignment))
    minimum = num_legal_values(csp, choice, assignment)
    max_degree = len(csp.neighbors[choice])
    for v in csp.variables:
        if (v not in assignment) and (num_legal_values(csp, v, assignment) == minimum):
            if max_degree < len(csp.neighbors[v]):
                choice = v
    return choice
        ...
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                ...
                if inference(csp, var, value, assignment, removals):
                    result, num = backtrack(assignment)
                    assignments += num
