 ...
 *  @author Kevin Wayne
 */
public class BandwidthDijkstra {
     ...
     */
    public BandwidthDijkstra(Network G, int s) {
        for (Edge e : G.edges()) {
            if (e.getTime() < 0)
    ...
    private void relax(Edge e) {
        int v = e.from(), w = e.to();
        if (distTo[w] > distTo[v] + (1.0 / (double) e.getBandwidth())) {
            distTo[w] = distTo[v] + (1.0 / (double) e.getBandwidth());
            edgeTo[w] = e;
    ...
    // check optimality conditions:
    // (i) for all edges e:            distTo[e.to()] <= distTo[e.from()] + (1.0 / (double) e.getBandwidth())
    // (ii) for all edge e on the SPT: distTo[e.to()] == distTo[e.from()] + (1.0 / (double) e.getBandwidth())
    private boolean check(Network G, int s) {

        // check that edge weights are nonnegative
        for (Edge e : G.edges()) {
            if ((1.0 / (double) e.getBandwidth()) < 0) {
        ...
        // check that all edges e = v->w satisfy distTo[w] <= distTo[v] + (1.0 / (double) e.getBandwidth())
        for (int v = 0; v < G.getNumNodes(); v++) {
            for (Edge e : G.adj(v)) {
                int w = e.to();
                if (distTo[v] + (1.0 / (double) e.getBandwidth()) < distTo[w]) {
        ...
        // check that all edges e = v->w on SPT satisfy distTo[w] == distTo[v] + (1.0 / (double) e.getBandwidth())
